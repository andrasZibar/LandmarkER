...
public final class GmapsJsonUtils {
    ...
    private static final String GMAPS_LONGITUDE_PARAM = "lon";
    ...
    private static final String GMAPS_LOCATION_PHOTODATA_PARAM = "photo";
    ...
    public static Landmark[] getLandmarkObjectsFromJson(String jsonString){
        Landmark[] landmarks;
        JSONArray result;
        ...
        try {
            ...
            if (result.length() != 0){
                for (int i = 0; i < result.length(); i++) {
                    ...
                    String photoID = currentLandmark.getJSONObject(GMAPS_LOCATION_PHOTODATA_PARAM).getString(GMAPS_LOCATION_PHOTOID_PARAM);

                    landmarks[i] = new Landmark(lat, lon, locationID, name, photoID);
                }
            } else {
